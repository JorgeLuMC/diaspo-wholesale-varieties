...
function App() {
  ...
  const addToCart = async (item) => {
    ...
    try {
      await axios.post("http://localhost:3006/cartItems", item).then((res) => {
        if (res !== undefined) {
  ...
  return (
    ...
    <BrowserRouter>
      <MyContext.Provider value={value}>
        {
          isLoading===true && <div className='loader'><img alt='loader' src={Loader}/></div>
        }
        ...
        <Routes>
          ...
          <Route exact={true} path="/:id" element={<Listing data={data.productData} single={true} />} />
          <Route exact={true} path="/:id/:id" element={<Listing data={data.productData} single={false} />} />
